...
@property (nonatomic, assign) BOOL connectionReadyNotified;
...
//     [[NSUserDefaults standardUserDefaults] synchronize];
     ...
     self.connectionReadyNotified = NO;
...
- (void)startBleScan {
     ...
     if (self.centralManager.state != CBManagerStatePoweredOn) {
          ...
          if (self.centralManager.state != CBManagerStateUnknown &&
              self.centralManager.state != CBManagerStateResetting &&
              [self.managerDelegate respondsToSelector:@selector(connectBluetoothFailWithMessage:)]) {
               [self.managerDelegate connectBluetoothFailWithMessage:[self centralManagerStateDescribe:self.centralManager.state]];
          }
          return;
     }
...
- (void)sendDataToBle:(NSData *)data {
     if (data == nil || self.peripheral == nil || self.myCharacteristic == nil ||
         self.peripheral.state != CBPeripheralStateConnected) {
          NSLog(@"Cannot write BLE data before the R2 UART characteristic is ready.");
          return;
     }
     // NSLog(@"sendData");
...
- (void)centralManagerDidUpdateState:(CBCentralManager *)central {
     if (central.state != CBManagerStatePoweredOn &&
         central.state != CBManagerStateUnknown &&
         central.state != CBManagerStateResetting &&
         [self.managerDelegate respondsToSelector:@selector(connectBluetoothFailWithMessage:)]) {
          self.connectDevice = NO;
          [self.managerDelegate connectBluetoothFailWithMessage:[self centralManagerStateDescribe:central.state]];
     }
                  ...
                  RSSI:(NSNumber *)RSSI
{
     ...
     if (isDebug && advertisementData.description.length > 0) {
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateNotificationStateForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error
{
     if (error != nil) {
          ...
          if ([self.managerDelegate respondsToSelector:@selector(connectBluetoothFailWithMessage:)]) {
               [self.managerDelegate connectBluetoothFailWithMessage:error.localizedDescription];
          }
          [self.centralManager cancelPeripheralConnection:peripheral];
          return;
     }
     if (characteristic != self.notifyCharacteristic || !characteristic.isNotifying ||
         self.myCharacteristic == nil || self.connectionReadyNotified) {
          return;
     }

     self.connectDevice = YES;
     self.connectionReadyNotified = YES;
...
- (void) getKeyData:(Byte*) data {
     ...
     if (flag == 9) {
          NSLog(@"按下扫描按键");
          [self.managerDelegate rfidConfigCallback:[NSString stringWithFormat:@"%d", keyCode] function:0xe6];
     }
...
- (void)processData:(NSData *)uhfData cmd:(int)cmd{
     ...
     // 设置过滤
          ...
          NSLog(@"设置过滤返回=%d", res);
          [self.managerDelegate rfidSetFilterCallback:@"过滤" isSuccess:res];
     }
     // 设置盘点模式
     else if (cmd == 0x71) {
          Byte *byteData = (Byte*)[uhfData bytes];
...
- (NSString *)centralManagerStateDescribe:(CBManagerState)state {
     ...
     switch (state) {
          case CBManagerStateUnauthorized:
               descStr = @"Bluetooth permission is denied";
               break;
          case CBManagerStatePoweredOff:
               descStr = @"Bluetooth is turned off";
               break;
          case CBManagerStateUnsupported:
               descStr = @"Bluetooth Low Energy is not supported";
               break;
          case CBManagerStateResetting:
               descStr = @"Bluetooth is resetting";
               break;
          case CBManagerStateUnknown:
               descStr = @"Bluetooth state is unknown";
               break;
          default:
               descStr = @"Bluetooth is unavailable";
               break;
     }
     return descStr;
}
...
-(void) getBlutoothList {
     // 2EDCDAAC-26CE-2B9B-4248-F9468812CE4A
     
     NSLog(@"getBluetoothList----");
     
     NSArray<CBUUID *> *serviceUUIDs = @[[CBUUID UUIDWithString:@"2EDCDAAC-26CE-2B9B-4248-F9468812CE4A"]];
